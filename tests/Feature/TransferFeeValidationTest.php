...
class TransferFeeValidationTest extends TestCase
{
    use RefreshDatabase;

    public function test_transfer_fee_cannot_exceed_amount(): void
    {
        ...
        $this->actingAs($user)
            ->postJson("/api/households/{$household->id}/transactions", [
                ...
                'type' => 'transfer',
                'description' => 'Too much fee',
                'amount_minor' => 1000,
                'transfer_fee_minor' => 1001,
                'exchange_rate' => 3.75,
                'transaction_date' => now()->toDateString(),
                'created_by' => $user->id,
            ])
            ->assertStatus(422)
            ->assertJsonValidationErrors(['transfer_fee_minor']);
    }
